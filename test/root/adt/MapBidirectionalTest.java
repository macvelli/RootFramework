...
/**
 * Test the {@link MapBidirectional} class.
 *
 * @author Edward Smith
 * @version 0.5
 * @since 0.5
 */
public final class MapBidirectionalTest extends TestCase {

	private MapBidirectional<String, String> map;

	public MapBidirectionalTest() {
		super("MapBidirectional");
	}

	@Override
	@Before
	public void setUp() throws Exception {
		this.map = new MapBidirectional<>();
	}
	...
	public void testClone() {
		...
		final MapBidirectional<String, String> m = this.map.clone();
	...
	public void testConstructorCapacity() {
		MapBidirectional<String, String> m = new MapBidirectional<>(17);
		assertEquals(0, m.size());
		assertEquals(32, m.getCapacity());
		assertEquals(43, m.keyValueMap.table.length);

		// Test minimum capacity of 8
		m = new MapBidirectional<>(7);
		assertEquals(0, m.size());
		assertEquals(8, m.getCapacity());
		assertEquals(11, m.keyValueMap.table.length);
	}
	...
	public void testConstructorDefault() {
		...
		assertEquals(11, this.map.keyValueMap.table.length);
	}
	...
	public void testConstructorMap() {
		...
		final MapBidirectional<String, String> m = new MapBidirectional<>(stringMap);
		assertEquals(2, m.size());
		assertEquals(8, m.getCapacity());
		assertEquals(11, m.keyValueMap.table.length);
		assertEquals("bar", m.get("foo"));
	...
	public void testEquals() {
		final MapBidirectional<String, String> mapBidirectional = new MapBidirectional<>();

		assertTrue(this.map.equals(mapBidirectional));

		this.map.put("foo", "bar");

		assertFalse(this.map.equals(mapBidirectional));

		mapBidirectional.put("foo", "bar");
	...
	public void testGetKey() {
		assertNull(this.map.getKey("bar"));
		this.map.put("foo", "bar");
		assertEquals("foo", this.map.getKey("bar"));
	}
	...
	public void testPut() {
		...
		assertEquals("efg", this.map.getKey("hij"));

		this.map.put("klm", "nop");
		assertEquals(9, this.map.size());
		assertEquals(16, this.map.getCapacity());
		assertEquals(23, this.map.keyValueMap.table.length);
		assertEquals("nop", this.map.get("klm"));
		assertEquals("klm", this.map.getKey("nop"));

		this.map.put("foo", "ugh");
		assertEquals(9, this.map.size());
		assertEquals("ugh", this.map.get("foo"));
		assertEquals("foo", this.map.getKey("ugh"));
		assertNull(this.map.getKey("bar"));
	}
